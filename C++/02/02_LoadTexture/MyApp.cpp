...
GLuint CMyApp::GenTexture()
{
    ...
    for (int i=0; i<256; ++i)
	{
        for (int j=0; j<256; ++j)
        {
			float d = sqrt((i - 128) * (i - 128) + (j - 128) * (j - 128));
			tex[i][j][0] = d > 50 && d < 100 ? 255 : 0;
			tex[i][j][1] = d > 50 && d < 100 ? 255 : 0;
			tex[i][j][2] = d > 50 && d < 100 ? 255 : 0;
        }
	}
...
void CMyApp::Render()
{
	...
	*/
	...
	glUniform1f(glGetUniformLocation(m_programID, "t"), abs( sin(SDL_GetTicks() / 2000.0 * 2 * M_PI) ));
