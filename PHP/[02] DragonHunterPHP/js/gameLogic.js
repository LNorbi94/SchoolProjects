...
function grow(table, size, sID, n = -1, m = -1)
{
	...
	if(size > 0 && activeScroll != undefined)
	{
		...
		var tail = createImageForTable('img/Tail.svg', 'Sárkány farka');
		if(sID == scrollID)
		{
...
function move(table)
{
	...
	activeWay = way;
	if (table.rows[headCoord.x + coords.x] != undefined
	&&	table.rows[headCoord.x + coords.x].cells[headCoord.y + coords.y] != undefined
	&& table.rows[headCoord.x + coords.x].cells[headCoord.y + coords.y].innerHTML.indexOf('Tail') == -1)
	{
		...
		if(empty || cont)
		{
			...
			if(tails.length > 0)
			{
				var img = createImageForTable('img/Tail.svg', 'Sárkány farka');
